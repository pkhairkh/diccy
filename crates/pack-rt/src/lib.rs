...
/// DICOM Tag (0x3004,0x000E)
pub const TAG_DOSE_GRID_SCALING: Tag = Tag(0x3004, 0x000E);
/// DICOM Tag (0x3004,0x000C)
pub const TAG_GRID_FRAME_OFFSET_VECTOR: Tag = Tag(0x3004, 0x000C);
...
/// DICOM Tag (0x3006,0x0039)
pub const TAG_ROI_CONTOUR_SEQUENCE: Tag = Tag(0x3006, 0x0039);
/// DICOM Tag (0x3006,0x0040)
pub const TAG_CONTOUR_SEQUENCE: Tag = Tag(0x3006, 0x0040);
/// DICOM Tag (0x3006,0x0050)
pub const TAG_CONTOUR_DATA: Tag = Tag(0x3006, 0x0050);
/// DICOM Tag (0x3006,0x0042)
pub const TAG_CONTOUR_GEOMETRIC_TYPE: Tag = Tag(0x3006, 0x0042);
/// DICOM Tag (0x3006,0x0046)
pub const TAG_NUMBER_OF_CONTOUR_POINTS: Tag = Tag(0x3006, 0x0046);
/// DICOM Tag (0x3006,0x002A)
pub const TAG_ROI_DISPLAY_COLOR: Tag = Tag(0x3006, 0x002A);
/// DICOM Tag (0x300A,0x0002)
pub const TAG_RT_PLAN_LABEL: Tag = Tag(0x300A, 0x0002);
/// DICOM Tag (0x300C,0x0060)
pub const TAG_REFERENCED_STRUCTURE_SET_SEQUENCE: Tag = Tag(0x300C, 0x0060);
...
/// Helper function for read_f64
pub fn read_f64(dataset: &Dataset, tag: Tag) -> Result<f64> {
...
/// Helper function for read_f64_array
pub fn read_f64_array<const N: usize>(dataset: &Dataset, tag: Tag) -> Result<[f64; N]> {
    let Some(raw) = read_str(dataset, tag)? else {
        return Err(missing_required_tag(tag));
    };
...
/// Helper function for read_f64_vec
pub fn read_f64_vec(dataset: &Dataset, tag: Tag) -> Result<Vec<f64>> {
    let Some(raw) = read_str(dataset, tag)? else {
        return Err(missing_required_tag(tag));
    };
    let parts: Vec<&str> = raw.split('\\').collect();
...
/// Helper function for read_rgb
pub fn read_rgb(dataset: &Dataset, tag: Tag) -> Result<Option<[u8; 3]>> {
    let Some(raw) = read_str(dataset, tag)? else {
        return Ok(None);
    };
    let parts: Vec<&str> = raw.split('\\').collect();
...
/// Helper function for read_referenced_structure_set_uid
pub fn read_referenced_structure_set_uid(dataset: &Dataset) -> Result<Option<String>> {
