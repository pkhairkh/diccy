// Auto-extracted from src/tests.rs
// S13-T8: Move inline tests to tests/ directories
// Originally gated behind #[cfg(all(test, feature = "workflow-main-tests"))]
...
fn classify_hl7_mllp_nack_maps_error_classes_deterministically() {
    ...
    assert_eq!(decode_code, "DVF.DICOM.DECODE_ERROR");
    assert_eq!(nf_ack, "AE");
...
fn denylist_precedence_applies_before_role_and_tenant_scope_checks() {
    ...
    let shared = build_sr_workflow_state(&worklist_path, &mpps_path, &sr_path, &sr_audit_path);
    {
        ...
        store.health.write().denylist_routes = vec!["/workflow/audit".to_string()];
    }
...
fn build_sr_workflow_state(
    ...
    let state = RuntimeState {
        ...
        hl7: Hl7State::new(Hl7RuntimeState {
            ...
            ..Hl7RuntimeState::default()
        }),
        tenant: TenantState::new(),
        worker: WorkerState::new(1, BTreeMap::new()),
...
fn auth_mode_defaults_to_deny_all() {
    ...
    let mode = WorkflowAuthMode::DenyAll;
    let request = HttpRequest {
        method: "GET".to_string(),
    ...
    assert!(!authorize(&request, &mode, "tls", None));
}
...
fn insecure_transport_rejected_even_when_auth_allows() {
    ...
    let mode = WorkflowAuthMode::AllowAll;
    let request = HttpRequest {
        method: "GET".to_string(),
        path: "/healthz".to_string(),
        query: BTreeMap::new(),
    ...
    assert!(!authorize(&request, &mode, "insecure", None));
}

#[test]
fn token_mode_requires_matching_header_on_tls() {
    // REQ-AUTH-300: token mode allows only exact token match.
    let mode = WorkflowAuthMode::Token;
    let auth_token = Some("secret-token");
    let mut headers = BTreeMap::new();
    ...
    assert!(!authorize(&request, &mode, "tls", auth_token));
...
fn hl7_connector_features_endpoint_reports_feature_flag_and_rollout() {
    ...
    let shared = build_sr_workflow_state(&worklist_path, &mpps_path, &sr_path, &sr_audit_path);
    {
        ...
        state
            ...
            .insert("enterprise_his".to_string(), Hl7ConnectorFeatureFlag::Disabled);
        state
            .hl7
            .lock()
...
fn hl7_connector_delivery_is_skipped_when_feature_disabled() {
    ...
    let shared = build_sr_workflow_state(&worklist_path, &mpps_path, &sr_path, &sr_audit_path);
    {
        ...
        state
            ...
            .lock()
            .connector_feature_flags
            .insert("lab".to_string(), Hl7ConnectorFeatureFlag::Disabled);
    }
...
fn hl7_connector_delivery_is_skipped_when_rollout_zero() {
    ...
    let shared = build_sr_workflow_state(&worklist_path, &mpps_path, &sr_path, &sr_audit_path);
    {
        ...
        state
            ...
            .lock()
            .connector_feature_flags
            .insert("lab".to_string(), Hl7ConnectorFeatureFlag::Enabled);
        state
            .hl7
            .lock()
            .connector_rollout_percent
...
fn reconciliation_jobs_enforce_interval_bounds_max_jobs_and_tenant_isolation() {
...

    {
        ...
        while state.hl7.lock().reconciliation_jobs.len() < MAX_RECONCILIATION_JOBS {
            ...
            let id = format!("recon-cap-{0:05}", state.hl7.lock().reconciliation_seq);
