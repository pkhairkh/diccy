...
fn concept_item(code: &str, scheme: &str, meaning: &str) -> Dataset {
    ...
    item.insert(Element::new(TAG_CODE_VALUE, Vr::Sh, Value::Str(code.to_string())).unwrap());
    item.insert(
        Element::new(
    ...
    item.insert(Element::new(TAG_CODE_MEANING, Vr::Lo, Value::Str(meaning.to_string())).unwrap());
    item
}
...
fn build_num_item(value: &str) -> Dataset {
    ...
    item.insert(
        Element::new(
            ...
            Vr::Sq,
            Value::Sequence(vec![concept_item("G-D7FE", "SRT", "Length")]),
        )
        .unwrap(),
    );
    ...
    measured
        .insert(Element::new(TAG_NUMERIC_VALUE, Vr::Ds, Value::Str(value.to_string())).unwrap());
    measured.insert(
        Element::new(
            TAG_MEASUREMENT_UNITS_CODE_SEQUENCE,
            Vr::Sq,
            Value::Sequence(vec![concept_item("mm", "UCUM", "millimeter")]),
        )
        .unwrap(),
    );
    item.insert(
...
fn build_text_item(text: &str) -> Dataset {
    ...
    item.insert(
        Element::new(
            TAG_CONCEPT_NAME_CODE_SEQUENCE,
            Vr::Sq,
            Value::Sequence(vec![concept_item("121106", "DCM", "Finding")]),
        )
        .unwrap(),
    );
...
fn build_code_item() -> Dataset {
    ...
    item.insert(
        Element::new(
            TAG_CONCEPT_NAME_CODE_SEQUENCE,
            Vr::Sq,
            Value::Sequence(vec![concept_item("121071", "DCM", "Interpretation")]),
        )
        .unwrap(),
    );
    item.insert(
        Element::new(
            ...
            Vr::Sq,
            Value::Sequence(vec![concept_item("R-404FB", "SRT", "Normal")]),
        )
        .unwrap(),
    );
    item
...
fn sr_nested_extraction_is_deterministic_across_num_text_and_code_items() {
    ...
    container
        .insert(Element::new(TAG_VALUE_TYPE, Vr::Cs, Value::Str("CONTAINER".to_string())).unwrap());
    container.insert(
        Element::new(
...
fn sr_extractors_fail_closed_for_non_finite_empty_and_missing_code_content() {
    ...
    non_finite.insert(
        Element::new(
            TAG_CONTENT_SEQUENCE,
            Vr::Sq,
            Value::Sequence(vec![build_num_item("NaN")]),
        )
        .unwrap(),
    );
    ...
    invalid_code_item
        .insert(Element::new(TAG_VALUE_TYPE, Vr::Cs, Value::Str("CODE".to_string())).unwrap());
    missing_code.insert(
        Element::new(
            TAG_CONTENT_SEQUENCE,
            Vr::Sq,
            Value::Sequence(vec![invalid_code_item]),
        )
        .unwrap(),
    );
