...
impl TriangleMesh {
    ...
    /// Return a reference to the Normals enum for inspection.
    pub fn normals_ref(&self) -> &Normals {
        &self.normals
    }

    /// Return mutable access to vertices.
    pub fn vertices_mut(&mut self) -> &mut Vec<[f64; 3]> {
        &mut self.vertices
    }

    /// Return mutable access to triangles.
    pub fn triangles_mut(&mut self) -> &mut Vec<[u32; 3]> {
        &mut self.triangles
    }

    /// Push a vertex onto the mesh, returning its index.
    pub fn push_vertex(&mut self, v: [f64; 3]) -> u32 {
        let idx = self.vertices.len() as u32;
        self.vertices.push(v);
        idx
    }

    /// Push a triangle onto the mesh (indices must be valid at validation time).
    ...
    pub fn surface_area(&self) -> f64 {
        ...
        for tri in &self.triangles {
            ...
            area += (cross_prod[0] * cross_prod[0]
...
pub fn marching_cubes(
    ...
    if config.target_label == 0 {
        return Err(mesh_error(
            "target_label must be non-zero (0 is background)",
        ));
    }
        ...
        for y in 0..height.saturating_sub(1) {
            for x in 0..width.saturating_sub(1) {
                ...
                let corner_values: [f64; 8] =
                    corner_labels.map(|cl| if cl == config.target_label { 1.0 } else { 0.0 });
...
pub fn encode_3d_model_iod(
    ...
    ds.insert(Element::new(
        Tag(0x0008, 0x0016),
        Vr::Ui,
        Value::Uid("1.2.840.10008.5.1.4.1.1.104.1".to_string()),
    )?);
    ...
    ds.insert(Element::new(
        Tag(0x0020, 0x0052),
        Vr::Ui,
        Value::Uid(frame_of_ref_uid.to_string()),
    )?);
    ...
    ds.insert(Element::new(
        Tag(0x0070, 0x0081),
        Vr::Lo,
        Value::Str(format!("3D model: {}", mesh.label)),
    )?);
    ...
    ds.insert(Element::new(
        Tag(0x0066, 0x0015),
        Vr::Ul,
        Value::I32(mesh.vertex_count() as i32),
    )?);
    ...
    ds.insert(Element::new(
        Tag(0x0066, 0x0016),
        Vr::Ul,
        Value::I32(mesh.triangle_count() as i32),
    )?);
    ...
    ds.insert(Element::new(
        Tag(0x0008, 0x2111),
        Vr::St,
        Value::Str("Generated by dicom-mesh marching cubes".to_string()),
    )?);
...
pub type MeshAuditCallback =
    std::sync::Arc<dyn Fn(dicom_audit::AuditEvent) -> Result<()> + Send + Sync>;
